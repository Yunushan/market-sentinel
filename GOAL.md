...
- Total markets: 68
- Implemented or partially implemented adapters: 43
- Verified-blocked/stub adapters: 25
- Market/event discovery supported: 43 yes, 25 no
- Alerts supported: 42 yes, 26 no
- Read-only price data supported: 41 yes, 27 no
- Orderbook reading supported: 23 yes, 45 no
- Paper trading supported: 40 yes, 28 no
- Live trading supported: 32 guarded/off by default, 36 no
...
- Article 90 Probable Official API Adapter: scope complete for repository-controlled integration; the Probable market/CLOB endpoints are represented by a fixture-backed adapter with event/market discovery, token prices, orderbooks, alerts, dry-run paper orders, guarded HMAC-authenticated signed-order submission, registry/config/GUI integration, and explicit copy-trading limitations. Real credentialed and funded verification remains external and is not claimed here.
- Article 91 Matchbook Official API Adapter: scope complete for repository-controlled integration; the Matchbook event/market, decimal-odds, runner-offer, session-login, and `/v2/offers` contracts are represented by a fixture-backed adapter with probability-normalized prices/orderbooks, alerts, dry-run paper orders, guarded session-authenticated live offers, registry/config/GUI integration, and explicit copy-trading limitations. Account, regional, fee/limit, and funded execution evidence remains external and is not claimed here.
- Article 92 DFlow Official Metadata/Trade API Adapter: scope complete for repository-controlled integration; nested event/market discovery, settlement-account outcome-mint normalization, probability prices, complementary YES/NO orderbook reconstruction, dry-run Trade API requests, and guarded submission of a user-signed Solana transaction through an explicitly configured RPC are represented by fixture-backed tests. API-key, Proof/KYC, jurisdiction, wallet, funded-balance, RPC, redemption, and asynchronous order-status evidence remains external and is not claimed here.
...
- Article 98 Hyperliquid HIP-4 Outcome Adapter: scope complete for repository-controlled integration; the official `outcomeMeta`, encoded outcome-asset, and `l2Book` contracts, binary side mapping, prices, orderbooks, dry-run orders, and guarded externally signed exchange payloads are represented by fixture-backed tests. Hyperliquid private-key handling, wallet signing, settlement, and funded execution remain external and are not claimed here.
- Article 99 Trueo Official Base Contract Adapter: scope complete for repository-controlled integration; the official Base `TruthMarketManager`/`TruthMarket` contract reads, immutable market fields, documented Uniswap V3 pool prices, dry-run orders, and guarded externally signed transaction submission are represented by fixture-backed tests. Trueo CLOB depth, wallet signing, settlement, and funded execution remain external and are not claimed here.
...
- Article 104 Reality.eth Question Adapter: scope complete for repository-controlled integration; the official Reality.eth question subgraph schema is exposed through a configured GraphQL adapter with question discovery, response-option listing, lifecycle status, alert-compatible metadata, independent fixtures, and explicit read-only safety errors. Prices, orderbooks, paper orders, wallet answer submission, and trading remain unsupported because Reality.eth is an oracle/question protocol.
...
- Article 108 Blocked-Market Freshness Audit: scope complete for this pass; PRDT Finance was re-checked against its current official site, GitBook contract list, public `PredictionFactory` source, and verified-chain pages without finding a current public factory/indexer contract that can safely back a fixture-tested adapter. Probo's official site now states that operations are closed, and Sporttrade's official site states that all wagering ceased on 2026-05-25; those catalog entries remain fail-closed with updated blocker evidence rather than being promoted through private or stale endpoints.
...
- Article 112 Coinbase Prediction Markets/Kalshi Read-only Adapter: scope complete for this pass; Coinbase's official launch and help documentation identify Kalshi as the prediction-market venue and direct users to Kalshi market outcomes. A fixture-backed alias now covers public discovery, contracts, prices, orderbooks, alerts, and local dry-run orders through Kalshi's documented market-data API. Coinbase-specific account execution and copy trading remain explicitly disabled because no supported Coinbase order API is published.
