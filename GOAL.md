...
- Article 55 Polymarket Live Validation Report Export and HTTP Smoke: add stored-report JSON export/open-by-key support, route-level HTTP tests for report GET/POST/DELETE behavior, and browser smoke coverage for the Live Safety report-history controls while keeping all payloads redacted and funded execution CLI-only.
