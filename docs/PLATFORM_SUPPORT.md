...
| Red Hat Enterprise Linux / UBI | RHEL-family source compatibility is continuously verified, but it is not fully certified as a desktop platform yet. | GitHub Actions runs clean dependency installation, Enterprise Linux smoke checks, `python app.py --smoke-test`, and `python verify.py` inside `registry.access.redhat.com/ubi8/python-312:latest`, `registry.access.redhat.com/ubi9/python-312:latest`, and `registry.access.redhat.com/ubi10/python-312-minimal:latest`. RHEL 7 era ABI compatibility receives the same checks in `quay.io/pypa/manylinux2014_x86_64:latest`. Desktop certification still requires a repeatable RHEL desktop runner or release/install evidence, rather than container-only validation. |
