...
| Working adapter or truthful stub for each market | The 46 implemented adapters live under `market_adapters/`; verified unsupported adapters use `VerifiedBlockedAdapter` and the exact reasons in [`BLOCKERS.md`](BLOCKERS.md). Zetarium World uses the reviewed BSC `PredictionMarket` deployment and Lamas Finance uses the official Solana Anchor `PricePredict`/`UpOrDown` programs; both remain guarded for live execution. | `tests/test_market_adapters.py` verifies implemented adapters, 22 verified blockers, capability flags, and clear operational errors. `tests/test_blockers_doc.py` verifies all catalog entries have blocker coverage. |
