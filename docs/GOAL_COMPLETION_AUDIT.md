...
| CLI-only operation | `market_sentinel_cli.py` exposes market discovery, contract listing, normalized prices/orderbooks/trades/candles, market safety/configuration, alerts, paper including durable position-mark refresh/clear, wallet, copy, analytics, durable leaderboard scan/status/export, Live Safety report/review/decision/proposal artifacts, and server commands without Tkinter. | `tests/test_cli.py` and the full verifier cover CLI parsing, normalized market-read serialization, local paper marks and report artifacts, state handling, exports, recovery, and status. |
