...
| Discovery, contracts, prices, history, alerts, paper, guarded live, and copy capability declarations | Capability values are declared in `market_adapters/catalog.py` and enforced by `market_adapters/base.py` plus each adapter. Metaculus official Community Prediction aggregation history is normalized as irregular point candles only when the authenticated post response includes accessible history; its forecast snapshots deliberately carry no fabricated volume and are not resampled into exchange OHLCV. Opinion's and Polymarket's documented price histories are normalized as flat OHLC points with no fabricated volume; the shared IBKR event-contract adapter now normalizes documented historical Last Trade OHLC bars for ForecastTrader, ForecastEx, and CME event contracts; Opinion's, Manifold's, Myriad's, and Hyperliquid HIP-4's official wallet-trade/event feeds support simulation-first copy intents; Opinion also has an optional official CLOB SDK limit/market order boundary with BNB-chain and credential gates. The desktop and web selected-market wallet paths use the selected adapter feed instead of assuming Polymarket. Guarded live boundaries include Opinion CLOB orders, Thales externally signed AMM transactions, Hedgehog externally signed `DepositV1` transactions, Omen/Gnosis externally signed FPMM transactions, Seer externally signed third-party-DEX transactions, MetaDAO externally signed Solana router transactions, Zeitgeist externally signed `HybridRouter` Substrate extrinsics, Zetarium externally signed BSC `placeBet` transactions, and Lamas externally signed Solana Anchor `predict` transactions. | `tests/test_market_adapters.py` validates capability keys and unsupported-operation behavior; adapter-specific tests cover supported parser/history/order paths, Metaculus binary/multiple-choice/numeric forecast-history normalization and access failures, Polymarket public price-history and authenticated trade validation, Opinion CLOB order validation and price history, Opinion/Manifold/Myriad/Hyperliquid wallet activity and copy simulation, and the Thales, Hedgehog, Omen, Gnosis, Seer, MetaDAO, Zeitgeist, Zetarium, and Lamas signed-transaction guardrails. |
