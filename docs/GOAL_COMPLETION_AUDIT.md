...
| Betfair account-recovery parity | Betfair's documented authenticated `listCurrentOrders`, `listClearedOrders`, `getAccountFunds`, and `getAccountDetails` feeds are exposed through the explicit CLI/API/React account allow-list. Current-order status/order-by/sort/date/market/runner filters, cleared-order status/roll-up/id/date filters, bounded paging, item descriptions, and wallet selection are validated before forwarding; raw upstream payloads remain lossless and account calls use Betfair's separate account JSON-RPC endpoint. | `tests/test_additional_official_adapters.py`, `tests/test_verification_fixtures.py`, `tests/test_cli.py`, `tests/test_web_api.py`, and the full verifier. |
