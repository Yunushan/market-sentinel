...
| Working adapter or truthful stub for each market | The 47 implemented adapters live under `market_adapters/`; verified unsupported adapters use `VerifiedBlockedAdapter` and the exact reasons in [`BLOCKERS.md`](BLOCKERS.md). FanDuel Predicts now has a fixture-backed read-only OG/CDNA alias while CME-listed contracts remain covered by the CME/IBKR adapter. Zetarium World uses the reviewed BSC `PredictionMarket` deployment and Lamas Finance uses the official Solana Anchor `PricePredict`/`UpOrDown` programs; all live paths remain guarded. | `tests/test_market_adapters.py` verifies implemented adapters, 21 verified blockers, capability flags, and clear operational errors. `tests/test_blockers_doc.py` verifies all catalog entries have blocker coverage. |
