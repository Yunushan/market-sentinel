...
| Working adapter or truthful stub for each market | The 48 implemented adapters live under `market_adapters/`; verified unsupported adapters use `VerifiedBlockedAdapter` and the exact reasons in [`BLOCKERS.md`](BLOCKERS.md). Nadex now has a fixture-backed read-only CDNA prediction-event alias; FanDuel Predicts has a fixture-backed read-only OG/CDNA alias while CME-listed contracts remain covered by the CME/IBKR adapter. Zetarium World uses the reviewed BSC `PredictionMarket` deployment and Lamas Finance uses the official Solana Anchor `PricePredict`/`UpOrDown` programs; all live paths remain guarded. | `tests/test_market_adapters.py` verifies implemented adapters, 20 verified blockers, capability flags, and clear operational errors. `tests/test_blockers_doc.py` verifies all catalog entries have blocker coverage. |
...
| Offline fixtures and verifier | `tests/fixtures/` contains adapter and Polymarket fixtures, including the official Crypto.com Predictions/CDNA, Nadex/CDNA alias, Fanatics Markets alias, FanDuel Predicts/OG alias, Coinbase Prediction Markets/Kalshi alias, Opinion wallet trades, Reality.eth question-subgraph, Gnosis/Omen, IBKR event-contract, Hedgehog HPL Parimutuel account, Frenzy `BetSettled` log, PRDT legacy `Prediction` RPC shapes, reviewed Zetarium BSC `PredictionMarket` RPC shapes, and Lamas Solana Anchor `RoundResult` RPC shapes; `verify.py` validates fixture JSON, named required fixtures, imports, metadata, docs, workflows, and the test suite. It also requires a complete fixture-directory/test-file mapping for every implemented adapter. | `tests/test_verifier_coverage.py`, `tests/test_crypto_com_predict_adapter.py`, `tests/test_fanatics_markets_adapter.py`, `tests/test_fanduel_predicts_adapter.py`, `tests/test_nadex_adapter.py`, `tests/test_coinbase_prediction_adapter.py`, `tests/test_legacy_web3_adapters.py`, `tests/test_additional_official_adapters.py`, `tests/test_hedgehog_markets_adapter.py`, `tests/test_frenzy_finance_adapter.py`, `tests/test_prdt_finance_adapter.py`, `tests/test_zetarium_adapter.py`, `tests/test_lamas_finance_adapter.py`, and `python verify.py --frontend-build --frontend-live-smoke`. |
