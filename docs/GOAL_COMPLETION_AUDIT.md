...
| Betfair cleared-order recovery parity | Betfair's documented authenticated `listClearedOrders` settlement feed is exposed through the explicit CLI/API/React account allow-list. Bet status, roll-up grouping, market/event/runner/bet identifiers, bounded paging, item descriptions, and settled-date ranges are validated before forwarding; the raw cleared-order payload remains lossless. | `tests/test_additional_official_adapters.py`, `tests/test_verification_fixtures.py`, `tests/test_cli.py`, `tests/test_web_api.py`, and the full verifier. |
