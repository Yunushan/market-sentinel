...
- Enterprise Linux verification through RHEL UBI 8/9/10, a RHEL 7-era manylinux2014 ABI container, and Rocky Linux 8/9/10 containers. The UBI and Rocky containers provision Tkinter and Xvfb, then run clean dependency installation, the Enterprise Linux checks, the full Tkinter widget lifecycle smoke under Xvfb, the Tkinter fallback smoke command, and the complete Python verifier. The manylinux2014 lane remains an explicitly non-desktop ABI compatibility check; container GUI evidence does not by itself certify a native desktop installation on that distribution.
