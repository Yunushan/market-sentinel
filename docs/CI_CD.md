...
- Create a protected `release` environment with explicit approval by the sole maintainer for production releases.
- Enable Dependabot alerts, secret scanning, push protection, and private vulnerability reporting.
- Apply branch protection to `main` with signed commits, required pull requests,
  stale-review dismissal, strict required CI checks, and no administrator bypass.
  The single-maintainer approval policy is in `docs/REPOSITORY_SETTINGS.md`.
