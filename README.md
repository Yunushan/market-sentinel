...
- `GET /api/markets/{market_id}/trades?contract_id=...` and `GET /api/markets/{market_id}/candles?contract_id=...&resolution=1h` expose normalized history for adapters that document those feeds (currently Kalshi, Hyperliquid candles, Manifold trades, Myriad trades, Opinion authenticated filled trades and price history, Polymarket price history, IBKR event-contract executions/candles, Betfair matched orders, Matchbook matched bets, Space, Iowa Electronic Markets archive candles, and SciCast Data Mart archive snapshots/trades; IBKR execution history covers the current day and six previous days from the selected authorized account; Opinion, Polymarket, and SciCast's one-value-per-timestamp feeds are represented as flat OHLC points; Opinion trade history requires an explicit wallet address plus API key; Polymarket trade history still requires explicit operator-supplied CLOB L2 headers; IEM and SciCast history are archive-only); unsupported adapters fail closed with a structured error.
