...
- `POST /api/markets/{market_id}/orders/{operation}` exposes only Betfair's guarded `cancel_orders`, `update_orders`, and `replace_orders` mutations. Requests require the adapter opt-in `betfair_order_management_enabled`, the shared live-safety gates, bounded documented instructions, and an explicit confirmation for global cancellation; the route is POST-only and never accepts an arbitrary upstream method or path.
