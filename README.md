...
- `POST /api/markets/{market_id}/orders/{operation}` exposes only explicitly allow-listed Betfair (`cancel_orders`, `update_orders`, `replace_orders`) and Kalshi (`cancel_order`, `batch_cancel_orders`, `amend_order`, `decrease_order`) mutations. Requests require the venue-specific order-management opt-in, shared live-safety gates, bounded documented payloads, and explicit operator confirmation; the route is POST-only and never accepts an arbitrary upstream method or path.
