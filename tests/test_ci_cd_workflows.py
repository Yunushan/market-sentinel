...
class CiCdWorkflowTests(unittest.TestCase):
    def test_ci_workflow_covers_python_frontend_and_artifacts(self) -> None:
        ...
        self.assertIn("xorg-x11-server-Xvfb", enterprise_linux)
        self.assertIn("python3.12-tkinter xorg-x11-server-Xvfb git", enterprise_linux)
        ...
        self.assertIn('if ! command -v Xvfb >/dev/null 2>&1; then', enterprise_linux)
        self.assertIn('Xvfb :99 -screen 0 1280x1024x24 -nolisten tcp', enterprise_linux)
        self.assertIn("export DISPLAY=:99", enterprise_linux)
        self.assertIn('"$PYTHON_BIN" app.py --gui-smoke-test', enterprise_linux)
        self.assertIn("Xvfb is required for desktop validation but is not installed.", enterprise_linux)
