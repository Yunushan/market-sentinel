...
class ProductionOperationsTests(unittest.TestCase):
    ...
    def test_proxy_and_governance_artifacts_require_authenticated_tls_access(self) -> None:
        ...
        self.assertIn("Single-maintainer authorization", repository_settings)
        self.assertIn("zero required approving reviews", repository_settings)
        ...
        self.assertIn("reviewed actionlint and gitleaks release binaries", repository_settings)
        self.assertIn("SHA-256 digests before execution", repository_settings)
        self.assertNotIn("gitleaks/gitleaks-action", repository_settings)
        self.assertNotIn("raven-actions/actionlint", repository_settings)
